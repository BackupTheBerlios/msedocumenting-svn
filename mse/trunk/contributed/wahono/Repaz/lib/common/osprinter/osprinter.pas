...
{              Core classes for OSPrinter                 }
{                                                         }
{            Originally file is mseprinter                }
...
uses
 ...
 mseevent,mseglob,mseguiglob,msemenus,mseedit,msegui,msewidgets,pagesetupdlg,
 osprinterstype,mseformatstr;
...
type
 ...
 public
   ...
   property raw_pixelperchar: integer read fpixelperchar write fpixelperchar default 7;
...
begin
 with reader do begin
  ...
  fpixelperchar:= readinteger('pixelperchar',fpixelperchar);
...
begin
 with writer do begin
  ...
  writeinteger('pixelperchar',fpixelperchar);
...
begin
 if ftitle='' then ftitle:='MSEGUI Printing';
end;
...
begin
 ...
 dia.pixelperchar.value:= fpixelperchar;
 ...
 if dia.show(true)=mr_ok then begin
  ...
  fpixelperchar:= dia.pixelperchar.value;
...
//   self.ppmm:= fppmm;
   ...
   if fprintorientation = pao_landscape then begin
    ...
    fboundingbox.right:= round((fdrawinfo.gc.size.cy/ppmm-fpa_marginbottom)*mmtoprintscale);
    fboundingbox.top:= round((fdrawinfo.gc.size.cx/ppmm-fpa_marginright)*mmtoprintscale);
   end
   else begin
    fboundingbox.left:= round(fpa_marginleft*mmtoprintscale);
    fboundingbox.bottom:= round(fpa_marginbottom*mmtoprintscale);
    fboundingbox.right:= round((fdrawinfo.gc.size.cx/ppmm-fpa_marginright)*mmtoprintscale);
    fboundingbox.top:= round((fdrawinfo.gc.size.cy/ppmm-fpa_margintop)*mmtoprintscale);
   end;
   fclientsize.cx:= fdrawinfo.gc.size.cx - round((fpa_marginleft+fpa_marginright)*ppmm);
   fclientsize.cy:= fdrawinfo.gc.size.cy - round((fpa_margintop+fpa_marginbottom)*ppmm);
  end;
  with fdrawinfo.gc.size do begin
   if cx > cy then begin 
